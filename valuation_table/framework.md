...
```python
# 1. Create repository (data source)
...
cfo_ttm = repository.get_ttm_cfo("GOOG")
capex_ttm = repository.get_ttm_capex("GOOG")
shares = repository.get_yearly_shares_count("GOOG")
oe0, sh0, b = preprocessor.prepare(cfo_ttm, capex_ttm, shares)

# 4. Create model with fixed parameters
...
# 5. Calculate single intrinsic value
iv = model.calculate_intrinsic_value(
    discount_rate=0.09,
    initial_growth_rate=0.08,
)

# 6. Or build sensitivity table
table_builder = SensitivityTableBuilder(
    min_initial_growth_rate=0.05,
    max_initial_growth_rate=0.12,
    min_discount_rate=0.08,
    max_discount_rate=0.12,
)
df = table_builder.build(valuation_model=model)
```
