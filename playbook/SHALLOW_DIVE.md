...
티커별로 `revenue_ttm`, `ebit_ttm`, `cfo_ttm`, `capex_ttm`, `shares_q`, `total_equity_q`, `total_assets_q`, `price_latest`, `market_cap_latest` 존재 확인.

**주의 — PIT 가격 vs 최신 가격**: Gold 패널에는 두 종류의 가격이 있다.

- `price` / `date` / `market_cap`: 공시일(filed) 기준 PIT 가격. 과거 시계열 분석·백테스트용.
- `price_latest` / `date_latest` / `market_cap_latest`: Silver 기준 최신 시장가. **Phase 7 이후 모든 현재 밸류에이션 계산에 이 값을 사용할 것.**
...
**입력**: 현재가(`price_latest`)·Gold 패널
**산출**: 대략치 지표 표 + PBR 역산

**현재가 = `price_latest`** (`date_latest` 기준). `price`(PIT)는 공시일 가격이므로 Phase 7 이후 밸류에이션에 사용 금지.
...
**현재가 기준**: `price_latest` / `market_cap_latest` 사용 (Phase 7과 동일).
